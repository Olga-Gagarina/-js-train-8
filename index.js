...
function checkArray(obj) {
  if (obj !== null && obj !== undefined){
    const objIsArr = Array.isArray(obj);
    return objIsArr;
  }else{
...
function createArray(...elements) {
  if (elements.length > 0){
    const resulCreattArray =Array.of(...elements) ;
      return resulCreattArray;
     }
    else{
      console.log(` Error`);
    }
...
function getElementAtIndex(arr, index) {
  ...
  if ( index.length !== 0){
    const elementIndex = arr.at(index);
    return elementIndex;
  }
  else{
    return undefined;
...
function findElementIndexes(arr, element) {
  ...
  const elementIndexes1 = arr.indexOf(element);
  const elementIndexes2 = arr.lastIndexOf(element);
  const indexes =[];
  if (elementIndexes1 !== -1){
    indexes.push(elementIndexes1);
  }
  if(elementIndexes1 !== elementIndexes2){
    indexes.push(elementIndexes2);
  }
  return indexes;
...
function customFill(arr, value, start = 0, end = arr.length) {
  if (!Array.isArray(arr) || start < 0 || start > arr.length || end > arr.length || end < 0)
...
const myArray = [3, 4, 5];
const result = customUnshift(myArray, 1, 2);
console.log(result); // Виведе: { initialLength: 3, newLength: 5, arr: [1, 2, 3, 4, 5] }
...
function customAt(arr, index) {
  ...
  if(typeof elementIndex === `string`){
    console.log (`Елемент є рядком  ${elementIndex}`);
  }
  if(typeof elementIndex === `number`){
    console.log (`Елемент є числом ${elementIndex}`);
  }
  if(typeof elementIndex === `object`){
    console.log (`Елемент є обєктом ${elementIndex}`);
  }
